...
<ShowPlatform
  platforms={{
    desktop: (
      <Callout type="info">
        Hi there! You're seeing this because you're on desktop right now. If you load this page on
        mobile, you'll see a different callout.
      </Callout>
    ),
    touch: (
      <Callout type="info">
        Hi there! You're seeing this because you're on mobile right now. If you load this page on
        desktop, you'll see a different callout.
      </Callout>
    ),
    bot: <Callout type="info">Hi there! You're seeing this because you're on a bot.</Callout>,
    fallback: (
...
With [Next.js headers](https://nextjs.org/docs/app/api-reference/functions/headers) we can easily access the `user-agent` header and use it to determine the platform that the user is on.
...
As the `<ShowPlatform />` component is rendered on the server, and the action is declared with `"use server"`, the client bundle never needs to handle anything related to the `Bowser` library or the `user-agent` header.
